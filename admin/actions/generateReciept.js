...
//   //         <p><strong>2.</strong> नगर पंचायत की आधिकारिक वेबसाइट <a href="https://bewar.npup.in/">www.bewar.npup.in</a> पर गृहकर, जलकर व जलमूल्य का भुगतान ऑनलाइन कर सकते हैं।</p>
//   //       </div>
...
// ============================= This is the new Puppeteer code =========================================
// services/pdfGenerator.js
import puppeteer from 'puppeteer';
import fs from "fs";
...
class BrowserPool {
  constructor() {
    ...
    this.pagePool = [];
    this.maxPages = 5; // Maximum concurrent pages
  }
...
async function loadFonts() {
  ...
  try {
    if (fs.existsSync(fontPath)) {
      ...
      devanagariFont = buffer.toString('base64');
    }
    if (fs.existsSync(fallbackFontPath)) {
      const buffer = await readFileAsync(fallbackFontPath);
      englishFont = buffer.toString('base64');
    }
    fontsLoaded = true;
...
// Create queue instance with max 3 concurrent PDF generations
const pdfQueue = new PDFQueue(3);
...
export const generateTaxBillPDF = async (property, tax) => {
  ...
  return pdfQueue.add(async () => {
    ...
    try {
      ...
      const query = { houseNumber: property.houseNumber };
      const preNagarNigamData = await NagarNigamProperty.findOne(query);
      ...
      // HTML Template (using cached fonts)
      const htmlTemplate = `
<!DOCTYPE html>
<html lang="hi">
<head>
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <table class="bill-table">
          ...
          <tbody>
            <tr>
              <td rowspan="2" class="arv-cell">वार्षिक मूल्यांकन<br>${arv}</td>
              <td>2025-2026</td>
              <td>${houseTax}</td>
              <td>${waterTax}</td>
              <td rowspan="8" class="remarks-cell">
              ...
              <td rowspan="2">जारी करने की तिथि<br>${formatDate(tax.createdAt)}</td>
            </tr>
            <tr>
              <td>बकाया</td>
              <td colspan="2">${totalPrevBakaya.toFixed(2)}</td>
            ...
            <tr>
              <td rowspan="2">मूल्यांकन तिथि<br>${formatDate(tax.effectiveFrom)}</td>
              <td>अधिभार</td>
              <td colspan="2">${interestAmount.toFixed(2)}</td>
              <td rowspan="2">अंतिम तिथि<br>${formatDate(tax.dueDate)}</td>
            </tr>
            <tr>
              <td>छूट</td>
              <td colspan="2">${discount}</td>
            ...
            <tr>
              ...
              <td class="label-cell">भुगतान कर</td>
              <td colspan="2">${paidAmount.toFixed(2)}</td>
              <td rowspan="2">कुल देय धनराशि<br><strong>₹${dueAmount.toFixed(2)}</strong></td>
            </tr>
            <tr>
              <td class="label-cell">देय धनराशि</td>
              <td colspan="2" class="total-row">${totalTaxToPay.toFixed(2)}</td>
        ...
        <div class="footer-notes">
          <p><strong>1.</strong> यदि संपत्ति के स्वामी/अध्यासी करदाता को इस बिल के संबंध में किसी प्रकार की कोई आपत्ति है तो प्रोविजनल बिल प्राप्ति दिनांक से 15 दिवस के अंदर नगर पंचायत ${nagarNigamName}, मैनपुरी के कर विभाग में लिखित रूप से दर्ज करा सकते है अन्यथा इस बिल को अंतिम मानकर वसूली की जाएगी</p>
          <p><strong>2.</strong> नगर पंचायत की आधिकारिक वेबसाइट <a href="https://ghiror.npup.in/">www.ghiror.npup.in</a> पर गृहकर, जलकर व जलमूल्य का भुगतान ऑनलाइन कर सकते हैं।</p>
        </div>
        <div class="signatures">
          <div class="signature-box">
            <div class="signature-line"></div>
...
`;
      ...
      console.log(`✅ PDF generated successfully in ${duration}ms (Queue: ${pdfQueue.getQueueSize()}, Running: ${pdfQueue.getRunningTasks()})`);
...
// ============== Queue Status Function (Optional) ==============
export const getPDFQueueStatus = () => {
  return {
    queueSize: pdfQueue.getQueueSize(),
    runningTasks: pdfQueue.getRunningTasks(),
    maxConcurrent: pdfQueue.maxConcurrent,
    browserConnected: browserPool.browser?.isConnected() || false
  };
};

// ============== Graceful Shutdown ==============
export const closePDFGenerator = async () => {
  console.log('🔄 Closing PDF generator...');
  await browserPool.closeBrowser();
  console.log('✅ PDF generator closed successfully');
};
